...
public class ADMMenuNode {
    ...
    //retrieves the menu item index within the Top Level (1) List
    public static Integer GetTopMenuIndexByName(String FocusedItem){
        ...
        if (!UIMenuListLevel1.containsKey(UIContext)){
            ...
            int count = 0;
            for (String item : UIMenuListLevel1.get(UIContext)){
                if (item==FocusedItem) return count;
                count++;
            }
            return 0;
        }
    ...
    //retrieves the menu item name within the Top Level (1) List
    public static String GetTopMenuItemByIndex(Integer Index){
        String UIContext = sagex.api.Global.GetUIContextName();
        if (!UIMenuListLevel1.containsKey(UIContext)){
    ...
    public static Boolean IsBlank(String Item){
        return Item.startsWith("BlankItem");
    }

    //used for 360 menu to pad before the first item as well as after the focused item
    public static List<String> GetTopMenuList(Integer FocusedIndex, Integer PadBlanks ){
        Collection<String> topList = GetMenuList(1);
        ...
        LOG.info("**** GetTopMenuList called with FocusPos '" + FocusPos + "' FocusedIndex '" + FocusedIndex + "' OddOffset '" + OddOffset + "' TopMenu '" + topList + "'");
        //fill the List with Blanks
        for (int i = 0; i < topList.size() + PadBlanks + OddOffset; i++){
            retList.add("BlankItem" + i);
        }
    ...
    public static Collection<String> GetMenuList(Integer Level, Integer BlanksBefore, Integer BlanksAfter ){
        Collection<String> retList = new LinkedList<String>();
        //add blanks before the list
        int counter = 0;
        for (int i = 0; i < BlanksBefore; i++){
            counter++;
            retList.add("BlankItem" + counter);
        }
        //add the menu items
        retList.addAll(GetMenuList(Level));
        //add blanks after the list
    ...
    public static Boolean PropertyLoad(PropertiesExt MenuItemProps){
        ...
        //find all MenuItem Name entries from the properties file
        Collection<String> MenuItemNames = GetMenuItemsList(MenuItemProps);
...
//    }
    ...
    public static void ChangeMenuLocationOverride(){
        ...
        if (!Override){
            //switching from Client to Override
            String OverrideFilePath = GetDefaultMenuLocationOverride();
            Boolean FileExists = (new File(OverrideFilePath)).exists();
            if (FileExists){
