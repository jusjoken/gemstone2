...
//    return Headers;}
    ...
    private static String FormatProp(Boolean bFlow, String PropSection, String PropName){
        String tProp = "";
        if (bFlow){
            tProp = Flow.GetFlowBaseProp(PropSection) + Const.PropDivider + PropName;
        }else{
            if (PropSection.equals(Const.SageRootProp)){
                //expect that the property name is a SageTV root property and NOT a Gemstone one
                tProp = PropName;
            }else{
    ...
    public static void SetOptionBase(Boolean bFlow, String PropSection, String PropName, String NewValue, Boolean serverProp){
        String tProp = "";
        tProp = FormatProp(bFlow,PropSection,PropName);
        //LOG.debug("SetOptionBase: property '" + tProp + "'");
    ...
    public static String GemstoneSTVFileName(){
        return Const.STVFileName;
    }
    ...
    //Note: this did not work when called from the gemstone.java as the context is not set yet at that time
    //so this is not in use right now
    //may consider a core change to see if I can auto load an STV after an STV plugin is installed
    public static void LoadGemstoneSTV(){
        //check to see if Gemstone STV is already loaded
        LOG.info("LoadGemstoneSTV: getting the context");
        UIContext tContext = new UIContext(sagex.api.Global.GetUIContextName());
        LOG.info("LoadGemstoneSTV: context '" + tContext + "'");
        //LOG.info("LoadGemstoneSTV: testing if Gemstone STV is loaded");
        //File currentSTVFile = new File(sagex.api.WidgetAPI.GetCurrentSTVFile(tContext));
        //LOG.info("LoadGemstoneSTV: currentSTVFile created '" + currentSTVFile + "'");
        //String currentSTV = sagex.api.Utility.GetFileNameFromPath(currentSTVFile);
        //LOG.info("LoadGemstoneSTV: currentSTV is '" + currentSTV + "'");
        //if (currentSTV + "" != GemstoneSTVFileName() + ""){
            LOG.info("LoadGemstoneSTV: Gemstone is not current so loading it");
            //clear the theme variable so it gets reloaded if needed
            //sagex.api.Global.AddGlobalContext(tContext,"gCurThemeLoaded",null);
            //mark global variables as needing to be reloaded
            //sagex.api.Global.AddGlobalContext(tContext,"gGlobalVarsAreSet",null);
            LOG.info("LoadGemstoneSTV: Global variables reset");
            //Placeshifter check here
            //Load the STV File
            File GemSTVFile = new File(GemstoneSTVFile());
            LOG.info("LoadGemstoneSTV: created STV File for loading");
            sagex.api.WidgetAPI.LoadSTVFile(tContext,GemSTVFile);
            LOG.info("LoadGemstoneSTV: LoadSTVFile called");
        //}
    }
    ...
    // Determine if the 2015 YouTube Plugin is available and return the symbol to launch or null if not found
    public static String GetPluginYouTube(){
        String PluginID = "youtube";
        String WidgetSymbol = "TMIR-91180";
        // check to see if the ADM Plugin is installed
        Object[] FoundWidget = new Object[1];
        FoundWidget[0] = sagex.api.WidgetAPI.FindWidgetBySymbol(new UIContext(sagex.api.Global.GetUIContextName()), WidgetSymbol);
        if (sagex.api.PluginAPI.IsPluginEnabled(sagex.api.PluginAPI.GetAvailablePluginForID(PluginID)) || FoundWidget[0]!=null){
            return WidgetSymbol;
        }
        return null;
    }
    ...
    //TODO: this seems to fire from the STV ApplicationStarted hook but BEFORE the plugins are fully handled
    public static Boolean HandleNonCompatiblePlugins(){
        sagex.api.Global.DebugLog("HandleNonCompatiblePlugins: start checking for non-compatible plugins: " + LogInfo());
        ...
        UIContext tUI = new UIContext(sagex.api.Global.GetUIContextName());
        sagex.api.Global.DebugLog("HandleNonCompatiblePlugins: GetUIContextName: " + tUI);

        Object test = PluginAPI.GetAvailablePluginForID("DiamondSTVi");
        if (test==null){
            sagex.api.Global.DebugLog("HandleNonCompatiblePlugins: Plugin System NOT ready");
            return false;
        }

/*
        //List plugins
        sagex.api.Global.DebugLog("HandleNonCompatiblePlugins: GetAllAvailablePlugins: " + LogInfo());
        Object[] plugins = PluginAPI.GetAllAvailablePlugins();
        if (plugins != null && plugins.length > 0) {
            for (Object plugin : plugins) {
                sagex.api.Global.DebugLog("HandleNonCompatiblePlugins: Plugin '" + PluginAPI.GetPluginIdentifier(plugin)+ "' Installed '" + PluginAPI.IsPluginInstalled(plugin) + "' Enabled '" + PluginAPI.IsPluginEnabled(plugin) + "' C Installed '" + PluginAPI.IsClientPluginInstalled(plugin) + "'");
...
*/
        ...
        sagex.api.Global.DebugLog("HandleNonCompatiblePlugins: checking for CVF: " + LogInfo());
        Object thisPlugin = sagex.api.PluginAPI.GetAvailablePluginForID(tUI,"jusjokencvf");
        sagex.api.Global.DebugLog("HandleNonCompatiblePlugins: CVF: thisPlugin " + thisPlugin);
        ...
        if (sagex.api.PluginAPI.IsPluginEnabled(tUI,thisPlugin)){
            if(DisableForConflict){
                ...
                sagex.api.Global.DebugLog("HandleNonCompatiblePlugins: CVF found and disabled");
            }else{
                ...
                sagex.api.Global.DebugLog("HandleNonCompatiblePlugins: CVF found and System Error message created");
            }
        }else{
            sagex.api.Global.DebugLog("HandleNonCompatiblePlugins: checking for CVF - not found");
        }
        ...
        sagex.api.Global.DebugLog("HandleNonCompatiblePlugins: complete - checking for non-compatible plugins: " + LogInfo());
        return ReloadUI;
    }
